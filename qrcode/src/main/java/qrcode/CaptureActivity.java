...
import com.example.qrcode.CommonUtils;
import com.example.qrcode.R;
import com.example.qrcode.base.BaseAppCompatActivity;
...
import qrcode.decode.DecodeUtils;
import qrcode.utils.BeepManager;
import qrcode.utils.InactivityTimer;
...
public class CaptureActivity extends BaseAppCompatActivity implements SurfaceHolder.Callback {
    ...
    @Override
    protected int getLayoutId() {  return R.layout.activity_capture;
    }

    @Override
    protected void initView() {
        capturePreview = $(R.id.capture_preview);
        captureErrorMask = $(R.id.capture_error_mask);
        captureScanMask = $(R.id.capture_scan_mask);
        captureCropView = $(R.id.capture_crop_view);
        capturePictureBtn = $(R.id.capture_picture_btn);
        captureLightBtn = $(R.id.capture_light_btn);
        captureModeGroup = $(R.id.capture_mode_group);
        captureContainer = $(R.id.capture_container);
...
//                toForResult(CommonImagePickerListActivity.class, IMAGE_PICKER_REQUEST_CODE);
            }
        });
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        if (holder == null) {
            Log.e("LOG", "*** WARNING *** surfaceCreated() gave us a null surface!");
        }
        if (!hasSurface) {
            hasSurface = true;
//            initCamera(holder);
    ...
    public void handleDecode(String result, Bundle bundle) {
        ...
        if (!TextUtils.isEmpty(result) && CommonUtils.isUrl(result)) {
            Intent intent = new Intent(Intent.ACTION_VIEW);
            intent.setData(Uri.parse(result));
            startActivity(intent);
        } else {
            bundle.putString(ResultActivity.BUNDLE_KEY_SCAN_RESULT, result);
            to(ResultActivity.class, bundle);
        }
    }
    ...
    private void displayFrameworkBugMessageAndExit() {
        ...
        builder.setMessage("open_camera_error");
