...
public class PinYin4j {
    private static PinYin4j instance;
    public static PinYin4j getInstance() {
        if (instance == null) {
            instance = new PinYin4j();
        }
        return instance;
    }
     ...
     * @return
     */
    public  String getAllPinyinChar(String str){
        StringBuilder convert = new StringBuilder();
        Set<String> set1 = getAllPinyin(str);
        for (String word:set1) {
            convert.append(word);
        }
        return convert.toString().toUpperCase();
    ...
    public static <T extends Object> Set<T> array2Set(T[] tArray) {
        Set<T> tSet = new HashSet<>(Arrays.asList(tArray));
